...
"SELECT projects.title, SUM(pledges.amount), COUNT(projects.funding_goal BETWEEN pledges.amount) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING "
end
