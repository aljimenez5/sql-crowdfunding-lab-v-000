...
"SELECT users.name, SUM(pledges.amount - projects.funding_goal) FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY SUM(pledges.amount)"
end
