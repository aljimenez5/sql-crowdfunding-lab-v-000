...
"SELECT projects.title, SUM(pledges.amount) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING COUNT(projects.amount_pledge BETWEEN pledges.amount)"
end
